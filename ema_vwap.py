...
def log_trade(trade, symbol):
    """
    Logs trade details in a standardized format.
    """
    timestamp = trade.get('timestamp')
    if trade['action'] == 'BUY':
        message = (
            f"BUY  | {symbol} | Time: {timestamp} | Size: ${trade['position_value']:.2f} | "
            f"Shares: {trade['shares']:.2f} | Balance%: {trade['position_pct']:.2f}% | "
            f"Price: ${trade['price']:.2f} | Fees: ${trade['fees']:.4f} | New Balance: ${trade['new_balance']:.2f}"
        )
    else:  # SELL
        message = (
            f"SELL | {symbol} | Time: {timestamp} | PnL: ${trade['pnl']:.2f} | Return: {trade['pnl_pct']:.2f}% | "
            f"Price: ${trade['price']:.2f} | Fees: ${trade['fees']:.4f} | New Balance: ${trade['new_balance']:.2f}"
        )
    logger.info(message)
